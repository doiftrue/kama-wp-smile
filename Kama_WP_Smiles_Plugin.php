...
class Kama_WP_Smiles_Plugin {
	...
	function _smiles_replace_cb( $match ){
		...
		// first replace the full paternas with the file name - (:smile:)
		$filename = isset( $match[1] ) ? $match[1] : '';
		if( $filename ){
			if( in_array( $filename, $this->get_opt( 'all_sm' ), true ) ){
	...
	function footer_scripts(){
		...
		if( ( $post && $post->comment_status !== 'open' ) || ! is_singular() ){
			return;
		}
	 ...
	 * @return array
	 */
	public function get_dir_smile_names( $dir = '' ){
		$out = [];
